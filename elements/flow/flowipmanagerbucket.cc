...
FlowIPManagerBucket::FlowIPManagerBucket() : _verbose(1), _tables(0), _groups(0), _def_thread(0), _mark(false) {
...
int FlowIPManagerBucket::solve_initialize(ErrorHandler *errh) {
    ...
    for (int i = 0; i < _groups; i++) {
        sprintf(buf, "FlowIPManagerBucket%d", i);
