#ifndef CLICK_FLOWARPQUERIER_HH
#define CLICK_FLOWARPQUERIER_HH
...
FlowARPQuerier(IP, ETH, I<keywords>)
FlowARPQuerier(NAME, I<keywords>)
...
FlowARPQuerier may have one or two outputs. If it has two, then ARP queries
are sent to the second output.

FlowARPQuerier implements special behavior for 0.0.0.0, 255.255.255.255, multicast
...
ARP state.  By default FlowARPQuerier creates its own internal ARPTable and uses
...
Amount of time after which FlowARPQuerier will start polling for renewal.  0 means
...
=e
   ...
   a :: FlowARPQuerier(18.26.4.24, 00:00:C0:AE:67:EF);
   c[0] -> a[1];
   c[1] -> ... -> a[0];
   a[0] -> ... -> ToDevice(eth0);
...
instances of FlowARPQuerier.

FlowARPQuerier uses packets' destination IP address annotations, and can destroy
their next packet annotations.  Generated ARP queries have VLAN TCI
annotations set from the corresponding input packets.

FlowARPQuerier will send at most 10 queries a second for any IP address.

=h ipaddr rw

Returns or sets the FlowARPQuerier's source IP address.

=h broadcast r

Returns the FlowARPQuerier's IP broadcast address.
...
class FlowARPQuerier : public FlowSpaceElement<fcb_arpcache> { public:

    FlowARPQuerier() CLICK_COLD;
    ~FlowARPQuerier() CLICK_COLD;

    const char *class_name() const		{ return "FlowARPQuerier"; }
  ...
  private:
    ...
    inline Packet* handle_ip(Packet *p, fcb_arpcache* fcb, bool response = false);
    inline void handle_response(Packet *p);
