// -*- c-basic-offset: 4 -*-
#ifndef CLICK_STORETIMESEQRECORD_HH
#define CLICK_STORETIMESEQRECORD_HH
...
StoreTimeSeqRecord([I<keywords> OFFSET, DELTA])

=s timestamps

=d

StoreTimeSeqRecord element can be handy when computing end-to-end TCP/UDP/ICMP packet
delays.  The element embeds a timestamp and a sequence number into a packet and
adjusts the checksum of the packet.  Once the initial timestamp has been
placed into the payload of the packet, a time difference can be computed
once a packet passes through another StoreTimeSeqRecord element.  The data can be
...
it assumes that the next header field is TCP/UDP/ICMP.  If this is not the case, the
element will discard the packet.

Packet payload found after the TCP/UDP/ICMP header.  Note, the TCP/UDP/ICMP payload must be at least
...
=back
*/
class StoreTimeSeqRecord : public BatchElement {
	public:
		StoreTimeSeqRecord() CLICK_COLD;

		const char *class_name() const	{ return "StoreTimeSeqRecord"; }
