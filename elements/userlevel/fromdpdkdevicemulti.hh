#ifndef CLICK_FROMDPDKDEVICEMULTI_HH
#define CLICK_FROMDPDKDEVICEMULTI_HH
...
/*
=title FromDPDKDeviceMulti

=c

FromDPDKDeviceMulti(PORT [, QUEUE, N_QUEUES, I<keywords> PROMISC, BURST, NDESC])
...
use multiple FromDPDKDeviceMulti with the same PORT argument. Each
FromDPDKDeviceMulti will open a different RX queue attached to the same port,
and packets will be dispatched among the FromDPDKDeviceMulti elements that
...
Boolean.  FromDPDKDeviceMulti puts the device in promiscuous mode if PROMISC is
...
ThreadScheduler element will be shared among FromDPDKDeviceMulti elements and
...
class FromDPDKDeviceMulti;

class FromDPDKDeviceMulti : public RXQueueDevice {
public:

    FromDPDKDeviceMulti() CLICK_COLD;
    ~FromDPDKDeviceMulti() CLICK_COLD;

    const char *class_name() const { return "FromDPDKDeviceMulti"; }
...
#endif // CLICK_FROMDPDKDEVICEMULTI_HH
